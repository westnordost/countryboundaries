...
public class GeoJsonWriter
{
	...
	private void writeGeometry(JSONStringer b, Geometry g) throws JSONException
	{
		...
		else if (g instanceof Polygon)
		{
			...
			writeMultiCoordinates(b, createReversed(g));
		}
		else if (g instanceof MultiPolygon)
		{
			b.key(TYPE).value("MultiPolygon").key(COORDINATES);
			writeMultiMultiCoordinates(b, createReversed(g));
		}
	...
	/* The shell of a polygon in GeoJSON is defined in counterclockwise order, in JTS, it is the
	   other way round */
	private static Geometry createReversed(Geometry geometry)
	{
		Geometry r = (Geometry) geometry.clone();
		r.normalize();
		r.reverse();
		return r;
	}
