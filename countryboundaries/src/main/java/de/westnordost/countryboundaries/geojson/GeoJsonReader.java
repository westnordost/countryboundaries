...
public class GeoJsonReader
{
	...
	/** JTS imposes a restriction on polygons and multipolygons that GeoJSON does not:
	 *  Polygons contained in a multipolygon and holes within a polygon may only touch at single
	 *  points. Finding and merging linear rings that touch each other in a line is computationally
	 *  quite expensive, so if the input is guaranteed to not contain these geometries, then it
	 *  can be turned off. */
	public GeoJsonReader(boolean disableMergePolygons)
	{
		this.mergePolygons = !disableMergePolygons;
	}
	...
	private MultiPolygon createMultiPolygon(JSONArray coords) throws JSONException
	{
		 ...
		 *  well-defined."*/
		if(mergePolygons) mergePolygons(polygons);
	...
	private Polygon createPolygon(JSONArray coords) throws JSONException
	{
		...
		if (linearRings.length > 1)
		{
			...
			if(mergePolygons) mergeHoles(inner);
		}
